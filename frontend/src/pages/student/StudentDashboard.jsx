...
const StudentDashboard = () => {
  ...
  const updateQuizStatusAndRedirect = async (quizId, status, assignmentId) => {
    try {
      const response = await updateQuizStatus(quizId, status);
      if (response.status) {
        navigate(`/sharequiz/${quizId}?assignmentId=${assignmentId}`);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Enhanced Tabs - Now only Assignments tab */}
              ...
              {assignments.map((assignment) => (
                <div key={assignment._id} className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-lg border border-gray-200/50 transition-all duration-300 hover:shadow-xl hover:border-indigo-200">
                  <div className="p-6">
                    ...
                    <button
                      onClick={() => updateQuizStatusAndRedirect(assignment.quiz._id, 'in_progress', assignment._id)}
